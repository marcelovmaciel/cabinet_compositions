...
def scan(party):
 ...
 while True:
  params={'sgUe':'RN','cdMunicipio':'7220','sqPartido':party['id'],'currentPage':page,'pageSize':10000}
  try:
...
with concurrent.futures.ThreadPoolExecutor(max_workers=3) as ex:
 for result in ex.map(scan,parties):
  (P/('tse_natal_'+str(result['id'])+'.json')).write_text(json.dumps(result,ensure_ascii=False,indent=2))
