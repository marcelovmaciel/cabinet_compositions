...
def scan(task):
 party,city,city_id=task
 log={'party':party['sgPartido'],'id':party['id'],'city':city,'city_id':city_id,'pages':[],'matches':[]};page=0
 while True:
  params={'sgUe':'GO','cdMunicipio':city_id,'sqPartido':party['id'],'currentPage':page,'pageSize':10000}
  try:
...
with concurrent.futures.ThreadPoolExecutor(max_workers=3) as ex:
 for result in ex.map(scan,[(p,c,i) for c,i in [('ANÁPOLIS','2064'),('GOIÂNIA','2173')] for p in parties]):
  (P/('tse_go_'+result['city_id']+'_'+str(result['id'])+'.json')).write_text(json.dumps(result,ensure_ascii=False,indent=2))
