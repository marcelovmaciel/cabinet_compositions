...
P=pathlib.Path(__file__).parent/'sources'
URL='https://filia2-consulta.tse.jus.br/filia-consulta/rest/v1/relacao-filiados'
NAMES=['CARLOS HENRIQUE MENEZES SOBRAL','CARLOS HENRIQUE SOBRAL','CARLOS SOBRAL']
...
def scan(task):
 uf,municipio,party=task
 log={'uf':uf,'municipio':municipio,'party':party['sgPartido'],'id':party['id'],'pages':[],'matches':[]};page=0
 while True:
  ...
  try:
   ...
   for row in rows:
    if norm(row.get('nmEleitor')) in NAMES or hashlib.sha256(str(row.get('numCpf')).encode()).hexdigest() == '375761f745aab380838f0437580d38f94127924894c43b1e7fd1f82b087e674e':
...
parties=json.loads(pathlib.Path('cabinet_dataset/research/followup_bolsonaro_affiliations/tse_partidos.json').read_text())
tasks=[(uf,mun,p) for uf,mun,ids in [('DF','1778',[42])] for p in parties if p['id'] in ids]
with concurrent.futures.ThreadPoolExecutor(max_workers=3) as ex:
 for result in ex.map(scan,tasks):
  (P/('tse_'+result['uf']+'_'+str(result['id'])+'.json')).write_text(json.dumps(result,ensure_ascii=False,indent=2))
  print(result['uf'],result['party'],sum(x['records']for x in result['pages']),result.get('error','OK'),result['matches'],flush=True)
